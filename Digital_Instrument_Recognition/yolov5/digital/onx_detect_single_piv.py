...
def decimal_detection(img):
    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ret, thresh = cv2.threshold(img_gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    if img.shape[0] < 55:
        kernel_h = 3
    elif 55 < img.shape[0] < 150:
        kernel_h = 6
    else:
        kernel_h = 15
    ero_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, kernel_h))
...
if __name__ == '__main__':
    ...
    img = cv2.imread(r'./test_data/1.jpg')
    ...
    ##################################################检测小数点######################################################
    decimal_x = decimal_detection(img.copy()) # 返回小数点x坐标，记得是img.copy不是resize的，会出错
